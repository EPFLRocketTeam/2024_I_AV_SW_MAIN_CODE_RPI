...
void UART3::RegisterHandler(int packetId, HandlerFunction handler)
{
    handlers[packetId] = handler;
}

size_t UART3::AvailableBytesToPeek() const
{
...
void UART3::AdvanceReadIndex(size_t amount)
{
    readIndex = (readIndex + amount) % RING_BUFFER_SIZE;
}

std::optional<uint8_t> UART3::PeekUnstuff()
{
...
bool UART3::DiscardCurrentByteAndContinue()
{
    AdvanceReadIndex(1);
    return true;
}
...
uint8_t UART3::ComputeChecksum(const uint8_t *data, size_t data_size)
{
    uint8_t checksum = 0;
